...
**Live site → [signal-map-148.emergent.host](https://signal-map-148.emergent.host/)**
...
Every warm region on the field is a topic gaining attention on the open web right now. The hero loops a rolling 7-day timelapse so the shape of the week is legible at a glance. See it in motion at **[signal-map-148.emergent.host](https://signal-map-148.emergent.host/)**.
...
This app needs a live Pulse API and a small set of environment variables that are **not distributed with the repository**. If you have been granted access, request the current `.env` values from the maintainers. If you just want to see Pulse in action, visit the live site: **[signal-map-148.emergent.host](https://signal-map-148.emergent.host/)**.

The web frontend lives in `frontend/`:
